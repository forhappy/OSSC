 ...
 * =============================================================================
 *
 *       Filename:  client_delete_bucket-t.c
 *
 *    Description:  client_delete_bucket testcase.
 *
...
size_t client_delete_bucket_callback(void *ptr, size_t size, size_t nmemb, void *stream)
{
	size_t r = size * nmemb;
		...
		unsigned short *retcode)
{
	...
	if (curl != NULL) {
		...
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, client_delete_bucket_callback);
...
int main()
{
	...
	client_delete_bucket(client, "bucket-name002", NULL);
}
