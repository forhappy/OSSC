 ...
 * =============================================================================
 *
 *       Filename:  client_list_buckets-t.c
 *
 *    Description:  client_list_buckets testcase.
 *
...
size_t client_list_bucket_callback(void *ptr, size_t size, size_t nmemb, void *stream)
{
	size_t r = size * nmemb;
 ...
 * */
oss_object_t *
client_list_bucket(oss_client_t *client,
		unsigned short *retcode)//, const char *bucket_name)
{
	...
	if (curl != NULL) {
		...
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, client_list_bucket_callback);
