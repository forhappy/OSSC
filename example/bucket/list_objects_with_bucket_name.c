 ...
 * =============================================================================
 *
 *       Filename:  list_objects_with_bucket_name.c
 *
...
int main()
{
	...
	int i;
	oss_client_t *client =
	    client_initialize_with_endpoint(access_id, access_key, endpoint);
	const char *bucket_name = "bucket_example";

	oss_object_listing_t *object_listing =
	    client_list_objects_with_bucket_name(client, bucket_name, &retcode);
	printf("list_objects_with_bucket_name result:\n\n");
	if (retcode == 0) {
		printf("list_objects_with_bucket_name successfully.\n");
		printf
		    ("bucket_name = %s\nnext_marker = %s\nprefix = %s\nmarker = %s\nmax_keys = %d\ndelimiter = %s\nis_truncated = %d\n",
		     object_listing->bucket_name, object_listing->next_marker,
		     object_listing->prefix, object_listing->marker,
		     object_listing->max_keys, object_listing->delimiter,
		     object_listing->is_truncated);
		for (i = 0; i < object_listing->_counts_summaries; i++) {
			...
			printf
			    ("type = %s\netag = %s\nkey = %s\nlast_modified = %s\nsize = %ld\nstorage_class = %s\nid = %s\ndisplay_name = %s\n",
			     object_listing->summaries[i]->type,
			     object_listing->summaries[i]->etag,
			     object_listing->summaries[i]->key,
			     object_listing->summaries[i]->last_modified,
			     object_listing->summaries[i]->size,
			     object_listing->summaries[i]->storage_class,
			     object_listing->summaries[i]->owner->id,
			     object_listing->summaries[i]->owner->display_name);
		}
	} else {
